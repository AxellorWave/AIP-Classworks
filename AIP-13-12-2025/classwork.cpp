...
bool isMag(const Stud * s)
{
  return s->year > 4 && s ->year < 7;
}

bool isOver(r_t m, r_t rv);

const Subj ** popular_mag(size_t & r, const Subj ** ss, size_t k, r_t rv)
{
  const Subj ** mags = new const Subj * [k];
  size_t count = 0;

  for (size_t i = 0; i < k; ++i) {
    const Subj * s = ss[i];
    bool everyMag = true;
    for (size_t j = 0; j < s->people; ++j) {
      if (isMag(s->studs[j])) {
        r_t m {s->visited[i], s->classes};
        if (!isOver(m, rv)) {
          everyMag = false;
          break;
        }
